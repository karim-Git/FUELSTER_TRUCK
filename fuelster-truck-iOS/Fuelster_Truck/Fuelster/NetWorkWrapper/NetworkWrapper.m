...

{
    ...
    dispatch_async(default_queuet, ^{
        ...
        if ([service rangeOfString:@"register"].location != NSNotFound || [service rangeOfString:@"login"].location != NSNotFound || [service rangeOfString:@"forgot-password"].location != NSNotFound || [service rangeOfString:@"reset-password"].location != NSNotFound || [service rangeOfString:@"validate-truck"].location != NSNotFound) {
            [request setupRequestHeadersAllowSession:NO];
        }
        else {
            [request setupRequestHeadersAllowSession:YES];
        }
    ...
    dispatch_async(default_queuet, ^{
        ...
        for (int i = 0; i<[body  count]; i++) {
            ...
            if ([body[i] isKindOfClass:[NSData class]]) {
                ...
                [postbody appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data;name=\"%@\";filename=\"%@.%@\"\r\n",nameKeys[i],filename,type] dataUsingEncoding:NSUTF8StringEncoding]];
...
+(NSError *)errorWithResponse:(NSHTTPURLResponse *)response withData:(NSData *)data
{
    ...
    NSError *httpError = [ [ NSError alloc ] initWithDomain:kNetworkUtilitiesErrorDomain code:kNetworkUtilitiesHttpErrorCode userInfo:userInfoDict];
